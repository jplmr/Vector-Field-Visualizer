...
class Parser {
	...
	/**
	 * parses and evaluates the given expression On error, an error of type Error is thrown
	 */
	...
	/**
	 * Get the next character from the expression. The character is stored into the char expr_c. If
	 * the end of the expression is reached, the function puts zero ('\0') in expr_c.
	 */
	void getChar() {
		expr_pos++;
	...
	/**
	 * Get the first character from the expression. The character is stored into the char expr_c. If
	 * the end of the expression is reached, the function puts zero ('\0') in expr_c.
	 */
	void getFirstChar() {
		expr_pos = 0;
	...
	/***
	 * Get next token in the current string expr. Uses the Parser data expr, e, token, t, token_type
	 * and err
	 */
	...
	double parse_level2() throws Error {
		...
		while (op_id == OPERATOR.AND || op_id == OPERATOR.OR || op_id == OPERATOR.BITSHIFTLEFT
	...
	double parse_level3() throws Error {
		...
		op_id = get_operator_id(token);
		while (op_id == OPERATOR.EQUAL || op_id == OPERATOR.UNEQUAL || op_id == OPERATOR.SMALLER
				|| op_id == OPERATOR.LARGER || op_id == OPERATOR.SMALLEREQ
	...
	double parse_level5() throws Error {
		...
		op_id = get_operator_id(token);
		while (op_id == OPERATOR.MULTIPLY || op_id == OPERATOR.DIVIDE || op_id == OPERATOR.MODULUS
				|| op_id == OPERATOR.XOR) {
			getToken();
	...
	/**
	 * returns the id of the given operator treturns -1 if the operator is not recognized
	 */
	...
	double eval_operator(final OPERATOR op_id, final double lhs, final double rhs) throws Error {
		...
		case DIVIDE:
			return lhs / rhs;
		case MODULUS:
			return Functions.modulus(lhs, rhs);
		case XOR:
			return (int) lhs ^ (int) rhs;
		...
		case POW:
			...
			// level 7
		case FACTORIAL:
			return Functions.factorial(lhs);
		}
	...
	double eval_function(final String fn_name, final double value) throws Error {
		...
		if (fnUpper.equals("FACTORIAL")) {
			return Functions.factorial(value);
		}
	...
	/**
	 * Shortcut for getting the current row value (one based) Returns the line of the currently
	 * handled expression
	 */
	int row() {
		return -1;
	...
	/**
	 * Shortcut for getting the current col value (one based) Returns the column (position) where
